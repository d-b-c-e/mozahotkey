...
[PluginActionId("com.dbce.moza-streamdeck.maxtorque")]
public class MaxTorqueAction : KeyAndEncoderBase
{
    ...
    private async void InitializeDisplay()
    {
        try
        {
            ...
            if (MozaDeviceManager.Instance.TryInitialize())
            {
                ...
                // During startup grace period, treat invalid values (< 50) as "not connected yet"
                // since Moza Pit House may not be fully loaded. Valid range is 50-100.
                if (currentValue < 50 && IsInStartupGracePeriod)
                {
    ...
    private void UpdateDirectionIcon()
    {
        ...
        Connection.SetImageAsync($"Images/torqueIcon{iconSuffix}.png");
    }
